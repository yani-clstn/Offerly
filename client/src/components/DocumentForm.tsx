...
export default function DocumentForm({ applicationId, onAdded }: { applicationId: number; onAdded: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-3 pt-3 border-t border-[#DEDCD3] space-y-2">
      <div className="flex gap-2">
        <select
          value={type}
          onChange={(e) => setType(e.target.value as typeof type)}
          className="text-xs bg-cream border border-[#DEDCD3] rounded-lg px-2 py-1.5 text-navy focus:outline-none focus:border-terracotta"
        >
